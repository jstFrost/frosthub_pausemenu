  ...
  return (
    <Panel title={lang.assets_title} delay="fh-d2">
      <div className="mb-[12px] pl-[11px] border-l-2" style={{ borderColor: 'var(--accent)' }}>
        <div className="text-[20px] font-bold leading-tight tracking-[0.5px]">{formatMoney(cash + bank)}</div>
        <div className="fh-meta mt-[3px]">{lang.assets_total}</div>
      </div>

      <DataRow label={lang.assets_cash} value={formatMoney(cash)} accent />
      <DataRow label={lang.assets_bank} value={formatMoney(bank)} />
    </Panel>
  )
}
